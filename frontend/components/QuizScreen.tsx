...
const QuizScreen: React.FC = () => {
  ...
  const resolvedQuizType = quizType?.toString() ?? "PERFIL";
  ...
  const loadQuiz = async () => {
    try {
      setLoading(true);
      setError(null);
      const quizData = await quizService.getQuiz(resolvedQuizType);
      setQuiz(quizData);
    } catch (err: any) {
  ...
  const loadSavedProgress = async () => {
    try {
      const progress = await quizService.loadProgress(resolvedQuizType);
  ...
  const handleAnswer = (questionId: number, selectedOption: string) => {
    ...
    const newAnswer: QuizAnswer = { questionId, selectedOption };
  ...
  const handleNext = () => {
    ...
    if (
      ...
      !quizService.validateAnswer(
        currentQuestion,
        currentAnswer || ({} as QuizAnswer)
      )
    ) {
      Alert.alert(
        "Atenção",
        "Por favor, responda à pergunta antes de continuar."
      );
      return;
    }
  ...
  const handleSubmit = async () => {
    ...
    const unansweredQuestions = quiz.questions.filter((question) => {
      ...
      return !quizService.validateAnswer(
        question,
        ...
        [
          {
            ...
            onPress: () => {
              ...
              const stepIndex = quiz.questions.findIndex(
  ...
  const renderQuestion = (question: QuizQuestion) => {
    ...
    const isAnswered =
      currentAnswer && quizService.validateAnswer(question, currentAnswer);
  ...
  const renderNavigationButtons = () => {
    ...
    const hasAnswer =
      currentAnswer &&
      quizService.validateAnswer(currentQuestion, currentAnswer);
      ...
      <View style={styles.navigationContainer}>
        ...
        >
          <MaterialIcons
            name="arrow-back"
            size={24}
            color={currentStep === 0 ? "#ccc" : "#DC2626"}
          />
          <Text
            style={[
