...
async updateProfile(payload: Partial<Profile>): Promise<Profile> {
  const { data } = await this.api.patch<ApiResponse<UpdateProfileResponse>>('/profile', payload);
  if (!data.success) throw new Error(data.message || 'Erro ao atualizar perfil');
